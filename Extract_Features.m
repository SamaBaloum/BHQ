function X_row=Extract_Features(Date,accelerometer,activity_recognition,battery,bluetooth,calls,gyroscope,...
...
X_row(7)=count_wifi; % numbers of use of wifi in a day
%X_row(8)=wifi_strength; % mean strength in a day
X_row(8)=(size(wifi_suuid(wifi_suuid==1))/size(wifi_suuid))*100; % 'Not found' precntage in a day
else 
    X_row(7)=0;
    %X_row(8)=NaN;
    X_row(8)=0;
...
X_row(9)=count_bluetooth; % numbers of use of bluetooth in a day
%X_row(11)=bluetooth_strength; % mean bluetooth strength in a day
X_row(10)=(size(bluetooth_suuid(bluetooth_suuid==1))/size(bluetooth_state))*100; % 'Not found' precntage in a day
X_row(11)=(size(bluetooth_state(bluetooth_state==1))/size(bluetooth_state))*100;% 'on' precntage in a day
else 
    X_row(9)=0;
    %X_row(11)=NaN;
    X_row(10)=0;
...
X_row(13)=(size(location_state(location_state==1))/size(location_state))*100;% 'on' precntage in a day
else 
    X_row(12)=0;
    X_row(13)=0;
end
...
X_row(15)=light_level; % mean light strength in a day
else 
  X_row(14)=0; 
  X_row(15)=0; 
end
...
X_row(16)= mean(sqrt(acc_x.^2 +acc_y.^2+acc_z.^2));%accelerometer_energy
X_row(17)=sum(acc_z); 
X_row(18)=std(acc_x)+std(acc_y)+std(acc_z); % standard deviation in 3 axises 
X_row(19)=mean(real(ifft(fft(acc_y))));
else
X_row(16)= 0;
X_row(17)= 0;
X_row(18)= 0;
...
X_row(20)= mean(sqrt(gyro_x.^2 +gyro_y.^2+gyro_z.^2));%Gyroscope_energy
X_row(21)=median(gyro_y); 
X_row(22)=mean(gyro_x)+mean(gyro_y)+mean(gyro_z); 
X_row(23)=mean(real(ifft(fft(gyro_z))));
else
    X_row(20)=0;
    X_row(21)=0;
    X_row(22)=0;
...
X_row(25)=battery_level; % mean level in a day
X_row(26)=(size(battery_state(battery_state==1))/size(battery_state))*100;% 'on' precntage in a day
else 
    X_row(24)=NaN; 
    X_row(25)=NaN; 
    X_row(26)=NaN;
...
X_row(27)=count_screen_state; % numbers of use of screen_state in a day
X_row(28)=(size(screen_state_state(screen_state_state==1))/size(screen_state_state))*100;% 'on' precntage in a day
else
    X_row(27)=NaN;
    X_row(28)=NaN;
end
...
X_row(32)=size(TILTING,1)/size(cur_activity_recognition,1);%Tilting precentage
X_row(33)=size(IN_VEHICLE,1)/size(cur_activity_recognition,1); %In Vehicle precentage
X_row(34)=size(ON_FOOT,1)/size(cur_activity_recognition,1);%ON_FOOT precentage
else
    X_row(29)=0;
    X_row(30)=0;
    X_row(31)=0;
